...
###Using Profiles
A common use case for mobile application is to have different functionality sets depending on the target device. In the sample I have created an specific profile for phones. This means that when the application is loaded from a phone device, we are going to display a different view than when the app is loaded from a tablet or a browser in case we are testing.

For the unspecific device, we have a TabPanel with a few components such as the employee list, the employee dataview, the toolbar items for filtering, etc. But for the **Phone** profile, I have created a TabPanel too but without the searching/filtering functionality and I'm only displaying the employee list with the same functionality when a user selects an employee we display the information on the employee form.

You can check the profile guide from [here](http://docs.sencha.com/touch/2-1/#!/guide/profiles) to learn more about it.

####How to test the profile

1. Load the application with Chrome or Safari browser.
2. Open the web inspector.
3. Click on the gear icon located at the bottom right on the inspector screen.
4. Go to the _Overrides_ tab
5. Change the _User Agent_ to any phone browser (iPhone - iOS5 for instance)
6. Set the device metrics. (I usually just check the _fit in window_) 
